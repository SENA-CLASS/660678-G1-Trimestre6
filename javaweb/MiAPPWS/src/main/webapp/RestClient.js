...
// Model for Municipio entity
    models.Municipio = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/MiAPPWS/webresources/edu.co.sena.pruebagit2.miappws.municipio/",
        idAttribute: 'idMUNICIPIO',
        defaults: {
            nombreMunicipio: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('idMUNICIPIO');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/MiAPPWS/webresources/edu.co.sena.pruebagit2.miappws.municipio/';
            }
    ...
    // Collection class for Municipio entities
    models.MunicipioCollection = Backbone.Collection.extend({
        model: models.Municipio,
        url: "http://localhost:8080/MiAPPWS/webresources/edu.co.sena.pruebagit2.miappws.municipio/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.MunicipioCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-municipio-list-itemis template identifier for item
                        templateName: '#tpl-municipio-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.municipio = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.municipio,
                    // tpl-municipio-details is template identifier for chosen model element
                    templateName: '#tpl-municipio-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Municipio();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-municipio-details is a template identifier for chosen model element
                templateName: '#tpl-municipio-details',
